...
module.exports = function(apiInfo) {
  var
    ...
    apiVersion = apiInfo.version || 'v2',
    accessToken = apiInfo.accessToken,
    email = apiInfo.email,
    customHeaders = [],
    userAgent = 'player-management-client/1.0.0',
    ...
    logCurlRequest = function(method, json, url, _extraParams){
      var extraParams = _extraParams || '',
        ...
        authParam = accessToken?
          ...
          '-u ' + email + ' ',
        userAgentHeader = '-H \'User-Agent: ' + userAgent + '\' ';

      console.info();
      console.info('curl command:');
      console.info('> curl -k ' + '-X ' + method + ' ' +
        userAgentHeader + authParam + data + contentType + extraParams + ' ' + url);
    },
